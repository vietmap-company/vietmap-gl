...
describe('AttributionControl', () => {
    test('appears in bottom-right by default', () => {
        ...
        expect(
            map.getContainer().querySelectorAll('.vietmapgl-ctrl-bottom-right .vietmapgl-ctrl-attrib')
        ).toHaveLength(1);
    });
    ...
    test('appears in the position specified by the position option', () => {
        ...
        expect(
            map.getContainer().querySelectorAll('.vietmapgl-ctrl-top-left .vietmapgl-ctrl-attrib')
        ).toHaveLength(1);
    });
    ...
    test('appears in compact mode if compact option is used', () => {
        ...
        expect(
            container.querySelectorAll('.vietmapgl-ctrl-attrib.vietmapgl-compact')
        ).toHaveLength(1);
        ...
        map.addControl(attributionControl);
        expect(
            container.querySelectorAll('.vietmapgl-ctrl-attrib:not(.vietmapgl-compact)')
        ).toHaveLength(1);
    });
    ...
    test('appears in compact mode if container is less then 640 pixel wide and attributions are not empty', () => {
        ...
        expect(
            container.querySelectorAll('.vietmapgl-attrib-empty')
        ).toHaveLength(0);
    });
    ...
    test('compact mode control toggles attribution', () => {
        ...
        const container = map.getContainer();
        const toggle = container.querySelector('.vietmapgl-ctrl-attrib-button');

        expect(container.querySelectorAll('.vietmapgl-compact-show')).toHaveLength(1);

        simulate.click(toggle);

        expect(container.querySelectorAll('.vietmapgl-compact-show')).toHaveLength(0);
    ...
    test('is hidden if empty', async () => {
        ...
        expect(container.querySelectorAll('.vietmapgl-attrib-empty')).toHaveLength(1);
    });
    ...
    test('is not hidden if adding a source with attributtion', async () => {
        ...
        expect(container.querySelectorAll('.vietmapgl-attrib-empty')).toHaveLength(0);
    });
...
describe('AttributionControl test regarding the HTML elements details and summary', () => {
    describe('Details is set correct for compact view', () => {
        test('It should NOT contain the attribute open="" on first load.', () => {
            ...
            expect(map.getContainer().querySelectorAll('.vietmapgl-ctrl-attrib')[0].getAttribute('open')).toBeNull();
        });
        ...
        test('It SHOULD contain the attribute open="" after click on summary.', () => {
            ...
            expect(map.getContainer().querySelectorAll('.vietmapgl-ctrl-attrib')[0].getAttribute('open')).toBe('');
        });
    ...
    describe('Details is set correct for default view (compact === undefined)', () => {
        ...
        test('The attribute open="" SHOULD exist after resize from size > 640 to <= 640 and and vice versa.', () => {
            ...
            expect(map.getContainer().querySelectorAll('.vietmapgl-ctrl-attrib.vietmapgl-compact')).toHaveLength(1);
            expect(map.getContainer().querySelectorAll('.vietmapgl-ctrl-attrib')[0].getAttribute('open')).toBe('');

            Object.defineProperty(map.getCanvasContainer(), 'offsetWidth', {value: 641, configurable: true});
            map.resize();

            expect(map.getContainer().querySelectorAll('.vietmapgl-ctrl-attrib:not(.vietmapgl-compact)')).toHaveLength(1);
            expect(map.getContainer().querySelectorAll('.vietmapgl-ctrl-attrib')[0].getAttribute('open')).toBe('');
        ...
        test('The attribute open="" should NOT change on resize from <= 640 to another <= 640 if it is open.', () => {
            ...
            const attributionControl = new AttributionControl({});
            map.addControl(attributionControl);
            const toggle = map.getContainer().querySelector('.vietmapgl-ctrl-attrib-button');
            simulate.click(toggle);
