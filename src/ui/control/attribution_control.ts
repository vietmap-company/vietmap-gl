...
export class AttributionControl implements IControl {
    ...
    onAdd(map: Map) {
        ...
        this._container = DOM.create('details', 'vietmapgl-ctrl vietmapgl-ctrl-attrib');
        this._compactButton = DOM.create('summary', 'vietmapgl-ctrl-attrib-button', this._container);
        this._compactButton.addEventListener('click', this._toggleAttribution);
        this._setElementTitle(this._compactButton, 'ToggleAttribution');
        this._innerContainer = DOM.create('div', 'vietmapgl-ctrl-attrib-inner', this._container);
    ...
    _toggleAttribution = () => {
        if (this._container.classList.contains('vietmapgl-compact')) {
            if (this._container.classList.contains('vietmapgl-compact-show')) {
                this._container.setAttribute('open', '');
                this._container.classList.remove('vietmapgl-compact-show');
            } else {
                this._container.classList.add('vietmapgl-compact-show');
    ...
    _updateAttributions() {
        ...
        if (attributions.length) {
            ...
            this._container.classList.remove('vietmapgl-attrib-empty');
        } else {
            this._container.classList.add('vietmapgl-attrib-empty');
        }
        this._updateCompact();
    ...
    _updateCompact = () => {
        if (this._map.getCanvasContainer().offsetWidth <= 640 || this._compact) {
            if (this._compact === false) {
                this._container.setAttribute('open', '');
            } else if (!this._container.classList.contains('vietmapgl-compact') && !this._container.classList.contains('vietmapgl-attrib-empty')) {
                this._container.setAttribute('open', '');
                this._container.classList.add('vietmapgl-compact', 'vietmapgl-compact-show');
            }
        } else {
            this._container.setAttribute('open', '');
            if (this._container.classList.contains('vietmapgl-compact')) {
                this._container.classList.remove('vietmapgl-compact', 'vietmapgl-compact-show');
            }
        }
    };
