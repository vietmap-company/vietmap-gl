...
export class LogoControl implements IControl {
    ...
    onAdd(map: Map) {
        ...
        this._container = DOM.create('div', 'vietmapgl-ctrl');
        const anchor = DOM.create('a', 'vietmapgl-ctrl-logo');
        ...
        if (containerChildren.length) {
            ...
            if (this._map.getCanvasContainer().offsetWidth <= 640 || this._compact) {
                if (this._compact !== false) {
                    anchor.classList.add('vietmapgl-compact');
                }
            } else {
                anchor.classList.remove('vietmapgl-compact');
            }
        }
    };
