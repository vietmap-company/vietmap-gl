...
describe('popup', () => {
    ...
    test('Popup#getElement returns a .vietmapgl-popup element', () => {
        const map = createMap();
        ...
        expect(popup.getElement().classList.contains('vietmapgl-popup')).toBeTruthy();
    });

    test('Popup#addTo adds a .vietmapgl-popup element', () => {
        const map = createMap();
        const popup = new Popup()
            .setText('Test')
            .setLngLat([0, 0])
        ...
        expect(popup.isOpen()).toBeTruthy();
        expect(map.getContainer().querySelectorAll('.vietmapgl-popup')).toHaveLength(1);
    });
    ...
    test('Popup closes on close button click events', () => {
        ...
        simulate.click(map.getContainer().querySelector('.vietmapgl-popup-close-button'));
    ...
    test('Popup has no close button if closeButton option is false', () => {
        ...
        expect(
            popup.getElement().querySelectorAll('.vietmapgl-popup-close-button')
        ).toHaveLength(0);
    });
    ...
    test('Popup content can be set via setHTML', () => {
        ...
        expect(popup.getElement().querySelector('.vietmapgl-popup-content').innerHTML).toBe('<span>Test</span>');
    });
    ...
    test('Popup content can be set via setDOMContent', () => {
        ...
        expect(popup.getElement().querySelector('.vietmapgl-popup-content').firstChild).toBe(content);
    });
    ...
    test('Popup anchors as specified by the anchor option', () => {
        ...
        expect(popup.getElement().classList.contains('vietmapgl-popup-anchor-top-left')).toBeTruthy();
    });
    ...
    ([
        ...
        test(`Popup automatically anchors to ${anchor}`, () => {
            ...
            expect(popup.getElement().classList.contains(`vietmapgl-popup-anchor-${anchor}`)).toBeTruthy();
        });
    ...
    test('Popup automatically anchors to top if its bottom offset would push it off-screen', () => {
        ...
        expect(popup.getElement().classList.contains('vietmapgl-popup-anchor-top')).toBeTruthy();
    });
    ...
    test('Popup#addTo is idempotent (#1811)', () => {
        ...
        expect(popup.getElement().querySelector('.vietmapgl-popup-content').textContent).toBe('Test');
    });
    ...
    test('Popup#remove is idempotent (#2395)', () => {
        ...
        expect(map.getContainer().querySelectorAll('.vietmapgl-popup')).toHaveLength(0);
    });
    ...
    test('Pointer-tracked popup is tagged with right class', () => {
        ...
        ).toContain('vietmapgl-popup-track-pointer');
    });
    ...
    test('Pointer-tracked popup can be repositioned with setLngLat', () => {
        ...
        expect(
            popup._container.classList.value
        ).not.toContain('vietmapgl-popup-track-pointer');
        expect(
            map._canvasContainer.classList.value
        ).not.toContain('vietmapgl-track-pointer');
    });
