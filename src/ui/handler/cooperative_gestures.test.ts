...
describe('CoopGesturesHandler', () => {
    ...
    test('Does not zoom on wheel if no key is down', async () => {
        ...
        map._renderTaskQueue.run();
        expect(map.getContainer().querySelector('.vietmapgl-cooperative-gesture-screen.vietmapgl-show')).toBeInstanceOf(HTMLDivElement);
        ...
        expect(map.getContainer().querySelector('.vietmapgl-cooperative-gesture-screen.vietmapgl-show')).toBeNull();
        map.remove();
    });
